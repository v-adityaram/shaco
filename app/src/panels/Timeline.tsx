...
const sourceColor: Record<string, string> = {
  ELK: 'text-amber-300 border-amber-500/40',
  Kafka: 'text-purple-300 border-purple-500/40',
  Apigee: 'text-sky-300 border-sky-500/40',
  APIM: 'text-cyan-300 border-cyan-500/40',
  MFT: 'text-lime-300 border-lime-500/40',
  ServiceNow: 'text-rose-300 border-rose-500/40',
  DevOps: 'text-orange-300 border-orange-500/40',
}
      ...
      <ol className="space-y-0">
        {items.map((item) => (
          <li
            ...
            title={item.ts}
            className={[
              'relative border-l border-slate-700 py-2 pl-4',
          ...
          >
            ...
            <div className="flex flex-wrap items-center gap-2">
              <span className="font-mono-tight text-[11px] text-slate-400">{toIST(item.ts)}</span>
              <span
                className={`rounded border px-1.5 py-0.5 text-[9px] font-semibold ${
                  sourceColor[item.source] ?? 'text-slate-300 border-slate-600'
                }`}
              >
                {item.source}
              </span>
            ...
            <div className="mt-0.5 text-[12.5px] text-slate-300">{item.description}</div>
            <div className="font-mono-tight text-[10px] text-slate-600">{item.event_id}</div>
          </li>
        ))}
      </ol>
    </FactPanel>

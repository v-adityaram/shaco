  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4">
      <div className="w-full max-w-md rounded-xl border border-slate-700 bg-slate-900 p-5 shadow-2xl">
        <h3 className="text-sm font-semibold text-slate-100">Approve recovery action</h3>
        <p className="mt-2 rounded border border-dashed border-amber-500/30 bg-amber-500/5 px-3 py-2 text-[12px] text-amber-200/90">
          {proposal}
        </p>
        ...
        <label className="mt-3 block text-[11px] font-medium text-slate-400">
          ...
          <textarea
            ...
            rows={2}
            className="mt-1 w-full resize-none rounded border border-slate-700 bg-slate-950 px-2 py-1.5 text-sm text-slate-100 outline-none focus:border-sky-500"
          />
        </label>
        <div className="mt-4 flex justify-end gap-2">
          <button
            onClick={onClose}
            className="rounded px-3 py-1.5 text-[12px] text-slate-400 hover:text-slate-200"
          >
            Cancel
          </button>
          <button
            ...
            className="rounded bg-sky-600 px-3 py-1.5 text-[12px] font-medium text-white disabled:cursor-not-allowed disabled:bg-slate-700 disabled:text-slate-500"
          >
            Approve &amp; log
          </button>
        </div>
...
export function ApprovalRecordCard({ record }: { record: ApprovalRecord }) {
  return (
    <div className="rounded border border-emerald-500/30 bg-emerald-500/5 px-3 py-2 text-[12px] text-emerald-200">

...
- **Count only the incident's own flows, and name the status.** `exchanges_stuck` is the number of the incident's own exchanges that are failed or stuck. Take it from one event, use the figure for the right status (Failed versus InProgress are different numbers), and exclude exchanges of unrelated flows that appear in the same report. In `business_objects_at_risk`, state which status the number refers to.
- **Cite each event on one side only.** Do not list the same event under both `supports` and `contradicts` of one hypothesis. If it cuts both ways, put it on the side where it weighs more and state the nuance in `why`.
- **Calibrate confidence.** Use `High` only when the contradicting evidence is minor and the check that would discriminate between the leading hypotheses has already been done, or its result is in the evidence. If your own `contradicts` admits an untested key assumption, use `Moderate`.
- **Mark exactly one first symptom** in the timeline.
